...
RUN sed -i 's!/var/www/html!/var/www/html/alura_design_patterns/public!g' /etc/apache2/sites-available/000-default.conf
...
WORKDIR /var/www/html/alura_design_patterns

# Verifica se o arquivo composer.json existe, se não, copia um arquivo de exemplo
COPY composer.json /var/www/html/alura_design_patterns/composer.json

# Instale as dependências do Composer
RUN composer install --no-interaction || true

# Definir permissões adequadas para o diretório
RUN chown -R www-data:www-data /var/www/html/alura_design_patterns
RUN chmod -R 755 /var/www/html/alura_design_patterns
...
RUN useradd -ms /bin/bash appuser && \
    chown -R appuser:appuser /var/www/html/alura_design_patterns
